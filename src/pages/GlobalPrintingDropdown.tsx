...
export default function GlobalPrintingDropdown() {
  const [lightboxOpen, setLightboxOpen] = useState(false);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);

  const images = [
    'IMG-20211001-WA0016.jpg',
    'IMG-20211112-WA0007.jpg',
    'IMG-20211113-WA0005.jpg',
    'New.jpg',
  ];

  const basePath = '/images/Global-Printing-&-Packaging';
  ...
  return (
    <div className="w-full">
      {/* Header */}
      <div className="w-full p-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-t-lg shadow-lg">
        <div className="text-center">
          <h3 className="text-3xl font-bold">Global Printing & Packaging</h3>
        </div>
      </div>

      {/* Content */}
      <div className="p-6 bg-white border-l-4 border-r-4 border-b-4 border-emerald-200 rounded-b-lg">
        {/* Photo Gallery */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {images.map((image, index) => (
            ...
            >
              <img
                src={`${basePath}/${image}`}
                alt={`Global Printing & Packaging Project - Image ${index + 1}`}
        ...
        <ImageLightbox
          ...
          imageAlt={`Global Printing & Packaging - Image ${currentImageIndex + 1}`}
